...
<div class="yith-wcwl-add-to-wishlist add-to-wishlist-<?php echo esc_attr($product_id) ?>">
    <?php if( ! ( $disable_wishlist && ! is_user_logged_in() ) ): ?>
        <div aria-label="<?php echo esc_attr($atts['label']);?>" class="hint--bounce hint--top-left yith-wcwl-add-button <?php echo ( $exists && ! $available_multi_wishlist ) ? 'hide': 'show' ?>" style="display:<?php echo ( $exists && ! $available_multi_wishlist ) ? 'none': 'block' ?>">

            <?php yith_wcwl_get_template( 'add-to-wishlist-' . $template_part . '.php', $atts ); ?>

        </div>

        <div aria-label="<?php echo esc_attr($product_added_text);?>" class="hint--bounce hint--top-left yith-wcwl-wishlistaddedbrowse hide" style="display:none;">
        ...
        <div aria-label="<?php echo esc_attr($already_in_wishslist_text);?>" class="hint--bounce hint--top-left yith-wcwl-wishlistexistsbrowse <?php echo ( $exists && ! $available_multi_wishlist ) ? 'show' : 'hide' ?>" style="display:<?php echo ( $exists && ! $available_multi_wishlist ) ? 'block' : 'none' ?>">
