...
class Mapping {
	...
	public function get_products( $product_ids ) {
		...
		foreach ( $product_ids as $key => $pid ) {
			...
			//var_dump(wp_get_post_terms($pid, 'product_brand', array('fields' => 'names')));
			/*if ( $product->is_type( 'variable' ) && $product->has_child() )
			{
				$parent_id = $product->get_parent_id();
			}*/	
//var_dump(wp_get_post_terms($parent_id, 'product_brand', ['fields' => 'names']));
	...
	protected function process_for_provider( $product_obj, $index ) {
		...
		if ( 'google' === $this->config['provider'] ) {
			...
			$brands =  implode(', ', wp_get_post_terms($parent_id, 'product_brand', ['fields' => 'names']));

			$this->feed_body .= "<g:brand>{$brands}</g:brand>";

#echo implode(', ', wp_get_post_terms($product['id'], 'product_brand', ['fields' => 'names']));
#var_dump(wp_get_post_terms($product['id'], 'product_brand', array('fields' => 'names')));


#echo $this->feed_body;exit;
